Meteor.methods({
    dental_specialPayment: function (params) {
        ...
        var registerDoc = Dental.Collection.SpecialRegister.findOne({_id: self.specialRegister});
        ...
        if (registerDoc) {
            ...
            if (registerDoc) {
                registerDoc.paymentMethod.forEach(function (object) {
                    // Check with payment
                    var getPayment =Dental.Collection.SpecialPayment.findOne({specialRegisterId: self.specialRegister, paymentMethod:object.index});
                    if (getPayment) {
                        object.paymentDate = getPayment.paymentDate;
                        object.paidAmount = numeral(getPayment.paidAmount).format('$0,0.00');
                    }
