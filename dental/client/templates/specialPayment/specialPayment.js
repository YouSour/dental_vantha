...
Template.dental_specialPayment.helpers({
    ...
    selector: function () {
        ...
        return {specialRegisterId: registerId};
    }
});
...
Template.dental_specialPayment.events({
    ...
    'click .insert': function () {
        ...
        // Check last balance
        var paymentLast = Dental.Collection.Payment.findOne({registerId: data._id}, {sort: {_id: -1}});
        if (!_.isUndefined(paymentLast)) {
            data.total = paymentLast.balance;
        }

        Meteor.setTimeout(function () {
            alertify.payment(fa("plus", "Special Payment"), renderTemplate(Template.dental_specialPaymentInsert, data));
        }, 500);
    },
    'click .update': function () {
        var data = Dental.Collection.SpecialPayment.findOne({_id: this._id});
        alertify.payment(fa("pencil", "Special Payment"), renderTemplate(Template.dental_specialPaymentUpdate, data));
    },
    ...
    dental_specialPaymentInsert: {
        before: {
            insert: function (doc) {
                ...
                var getDate = paymentMethod.split(" | ");
                var date = getDate[0];
                doc._id = idGenerator.genWithPrefix(Dental.Collection.SpecialPayment, branchPre, 3);
                doc.paymentMethod = date;
    ...
    dental_specialPaymentUpdate: {
        before: {
            update: function (modifier) {
                ...
                var getDate = paymentMethod.split(" | ");
                var date = getDate[0];
                modifier.$set.paymentMethod = date;
