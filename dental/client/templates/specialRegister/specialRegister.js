...
Template.dental_specialRegister.events({
    ...
    'click .insert': function () {
        alertify.register(fa("plus", "Special Register"), renderTemplate(Template.dental_specialRegisterInsert)).maximize();
    },
    'click .update': function () {
        var data = Dental.Collection.SpecialRegister.findOne({_id: this._id});

        alertify.register(fa("pencil", "Special Register"), renderTemplate(Template.dental_specialRegisterUpdate, data)).maximize();
    },
    ...
    'click .show': function () {
        ...
        alertify.alert(fa("eye", "Special Register"), renderTemplate(Template.dental_specialRegisterShow, data));
    },
    'click .statusAction': function () {
        var self = this;
        // Close register
        if (self.status == "Active") {
            var data = Dental.Collection.SpecialRegister.findOne({_id: self._id});
            data.status = 'Close';
